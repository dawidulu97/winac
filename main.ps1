...
& {
    ...
    # Prefer local copies bundled with the app
    $LocalFiles = @(
        (Join-Path -Path $PSScriptRoot -ChildPath '1.cmd'),
        (Join-Path -Path $PSScriptRoot -ChildPath '2.cmd'),
        (Join-Path -Path $PSScriptRoot -ChildPath '3.cmd')
    )
    $response = $null
    $source = 'remote'
    foreach ($lf in $LocalFiles) {
        if (Test-Path $lf) {
            try {
                $response = Get-Content -LiteralPath $lf -Raw -ErrorAction Stop
                $source = 'local'
                Write-Host "Using local MAS script: $lf" -ForegroundColor Green
                break
            } catch {}
        }
    }
    ...
    if (-not $response) {
        # Fallback to original remote URLs if local files are missing
        $URLs = @(
        ...
        if ($response) { Write-Host "Using remote MAS script." -ForegroundColor Yellow }
    }
    ...
    # Verify script integrity only for remote downloads (skip for local bundled copies)
    if ($source -eq 'remote') {
    ...
    # Force window title and visible banner strings to 'Activator v1' before writing
    $patched = $response
    try {
        $patched = [Regex]::Replace($patched, '(?im)^\s*title\s+.*$', 'title Activator v1')
    } catch {}
    $patched = $patched -replace '(?i)ohook activation\s*3\.7', 'Activator v1'

    # Normalize line endings to CRLF and ensure a trailing newline (per MAS troubleshoot guidance)
    $normalized = ($patched -replace "`r?`n", "`r`n")
    if (-not $normalized.EndsWith("`r`n")) { $normalized += "`r`n" }
    $payload = "@::: $rand `r`n" + "title Activator v1`r`n" + $normalized
    $enc = New-Object System.Text.ASCIIEncoding
    [System.IO.File]::WriteAllText($FilePath, $payload, $enc)
    CheckFile $FilePath
    ...
    if ($psv -lt 3) {
        ...
        $p = saps -FilePath $env:ComSpec -ArgumentList "/c """"$FilePath"" -el -qedit $args""" -Verb RunAs -WindowStyle Hidden -PassThru
        $p.WaitForExit()
    }
    else {
        saps -FilePath $env:ComSpec -ArgumentList "/c """"$FilePath"" -el $args""" -Wait -Verb RunAs -WindowStyle Hidden
    }
